...
for (let i = 0; i < name1.length; i++) {
  if (name1.toLowerCase()[i] === "a") count1++;
}
...
for (let o = 0; o < story7.length; o++) {
  storyString7 += o !== story7.length - 1 ? story7[o] + " " : story7[o];
}
...
for (let q = 0; q < name9.length; q++) {
  if (name9[q] !== " ") {
    upperName9 += name9[q].toLocaleUpperCase();
  }
}
...
for (let s = 0; s < city11.length; s++) {
  if (city11.toLowerCase()[s] === "e") count11++;
}
...
for (let v = 0; v < apostles14.length; v++) {
  apostlesString14 +=
    v !== apostles14.length - 1 ? apostles14[v] + ", " : apostles14[v];
}
...
for (let w = 0; w < name15.length; w++) {
  if ("bcdfghjklmnpqrstvwxyz".includes(name15[w])) count15++;
}
...
for (let bb = 0; name20[bb] !== undefined; bb++) {
  if (name20[bb] !== undefined) length20++;
}
